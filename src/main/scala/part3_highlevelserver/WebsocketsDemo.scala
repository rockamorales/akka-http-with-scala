...
object WebsocketsDemo extends App {
  ...
  // Message: TestMessage vs BinaryMessage
  ...
  Http().newServerAt("localhost", 8080).bindFlow(websocketRoute)

  case class SocialPost(owner: String, contents: String)
  val socialFeed = Source(
    List(
      SocialPost("Martin", "Scala 3 has been announced"),
      SocialPost("Daniel", "A new Rok the JVM course is open"),
      SocialPost("Martin", "I killed Java.")
    )
  )

  val socialMessages = socialFeed.throttle(1, 2 seconds)
    .map(socialPost => TextMessage(s"${socialPost.owner} said: ${socialPost.contents}"))
